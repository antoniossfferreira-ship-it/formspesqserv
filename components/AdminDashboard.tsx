...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ responses, onExit, onClear }) => {
  ...
  // Cálculos dinâmicos para os gráficos com base nas respostas reais
  ...
  const methodologyStats = useMemo(() => {
    const counts: Record<string, number> = {};
    responses.forEach(r => {
      r.preferredMethodologies?.forEach(m => {
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={roleStats.length > 0 ? roleStats : [{ name: 'Sem dados', value: 0 }]}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-8 h-[300px]">
            <div className="w-full sm:w-1/2 h-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={methodologyStats.length > 0 ? methodologyStats : [{ name: 'Sem dados', value: 1 }]}
                    cx="50%"
                    cy="50%"
                    innerRadius={65}
                    outerRadius={95}
                    ...
                    dataKey="value"
                  >
                    {(methodologyStats.length > 0 ? methodologyStats : [{ name: 'Sem dados', value: 1 }]).map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={methodologyStats.length > 0 ? COLORS[index % COLORS.length] : '#f1f5f9'} />
                    ))}
                  </Pie>
                  <Tooltip 
                    contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}}
            ...
            <div className="w-full sm:w-1/2 space-y-3">
              {methodologyStats.length > 0 ? (
                methodologyStats.map((entry, index) => (
                ...
                <div className="text-slate-400 text-sm text-center py-10">Nenhuma preferência registrada.</div>
              )}
            </div>
          </div>
        </div>
